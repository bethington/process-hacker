...
typedef struct _RTL_BALANCED_NODE32
{
    union
    {
        WOW64_POINTER(struct _RTL_BALANCED_NODE *)
        Children[2];
        struct
        {
            WOW64_POINTER(struct _RTL_BALANCED_NODE *)
...
{
    WOW64_POINTER(PRTL_BALANCED_NODE)
    Root;
    WOW64_POINTER(PRTL_BALANCED_NODE)
    Min;
...
{
    WOW64_POINTER(struct _LDR_SERVICE_TAG_RECORD *)
    Next;
    ULONG ServiceTag;
...
{
    WOW64_POINTER(PSINGLE_LIST_ENTRY)
    Tail;
...
typedef struct _PEB32
{
    ...
    WOW64_POINTER(PVOID)
    WaitOnAddressHashTable[128];
} PEB32, *PPEB32;

C_ASSERT(FIELD_OFFSET(PEB32, IFEOKey) == 0x024);
C_ASSERT(FIELD_OFFSET(PEB32, UnicodeCaseTableData) == 0x060);
// C_ASSERT(FIELD_OFFSET(PEB32, SystemAssemblyStorageMap) == 0x204); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(PEB32, pImageHeaderHash) == 0x23c); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(PEB32, WaitOnAddressHashTable) == 0x25c); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(sizeof(PEB32) == 0x460); // Disabled due to Windows SDK compatibility issues
...
typedef struct _TEB32
{
    ...
    WOW64_POINTER(PVOID)
    ReservedForDebuggerInstrumentation[16];
    WOW64_POINTER(PVOID)
    SystemReserved1[36];
    ...
    WOW64_POINTER(PTEB_ACTIVE_FRAME)
    ActiveFrame;
    WOW64_POINTER(PVOID)
    FlsData;
...
// C_ASSERT(FIELD_OFFSET(TEB32, GdiBatchCount) == 0xf70); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(TEB32, TlsExpansionSlots) == 0xf94); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(TEB32, FlsData) == 0xfb4); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(TEB32, MuiImpersonation) == 0xfc4); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(TEB32, ReservedForCrt) == 0xfe8); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(FIELD_OFFSET(TEB32, EffectiveContainerId) == 0xff0); // Disabled due to Windows SDK compatibility issues
// C_ASSERT(sizeof(TEB32) == 0x1000); // Disabled due to Windows SDK compatibility issues
